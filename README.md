...
В финале recap не заканчивается набором цифр — пользователь видит персонально выбранный следующий шаг: например, вернуться к избранному или продолжить поиск в любимой категории. В MVP этот CTA демонстрирует продуктовую механику, а при интеграции должен вести на реальные сущности Авито.
...
- условный набор слайдов в зависимости от реальной активности профиля;
- воспроизводимые правила архетипов, достижений и рекомендаций;
- объяснение, почему пользователь получил конкретный архетип;
- безопасная публичная карточка по постоянной ссылке;
- состояния загрузки, ошибки и недостаточной активности;
- персональный следующий шаг, подготовленный для интеграции с сущностями Авито.
...
API-контракт описан независимо через OpenAPI.  
Серверные типы и HTTP-обвязка генерируются с помощью `ogen`.
...
Это позволяет держать API-контракт явно описанным и отделённым  
...
Благодаря этому одни и те же параметры создают одинаковый dataset,  
а сценарий можно воспроизводить при разработке, тестировании и демонстрации.

### Идемпотентная генерация

Повторный запрос итогов одного пользователя за один год  
не создаёт новый recap.

### Privacy by design

Личная и публичная версии recap представлены разными моделями.

Публичная версия формируется отдельно и содержит только заранее разрешённый  
набор информации.

### Graceful degradation

Если определённого вида активности у пользователя нет,  
соответствующий блок может быть пропущен.

Если активности слишком мало для осмысленного recap,  
система показывает отдельное состояние вместо генерации случайных результатов.
...
- ссылки на реальные объявления отсутствуют в `ListingRef` -  поэтому карточки рекомендаций и старого избранного пока являются  информационными и не ведут непосредственно на объявление ;
...
При интеграции в реальный продукт следующим шагом станет подключение  событий пользовательской активности и переход CTA на реальные сущности.
...
|---|---|---|---|---|
|**Ильяс Юнусов**|[@horizoonn](https://github.com/horizoonn)|[@arkvnaa](https://t.me/arkvnaa)|Backend, frontend, инфраструктура|инфраструктура, repository layer, сборка приложения и DI, unit- и интеграционные тесты, публичный recap, рекомендации и финальная интеграция frontend|
